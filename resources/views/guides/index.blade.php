...
@section('content')
    ...
    <div class="lg:flex lg:gap-6">
        {{-- Desktop Sidebar (>= lg) --}}
        <aside class="hidden lg:sticky lg:top-20 lg:block lg:h-[calc(100vh-6rem)] lg:w-80 lg:shrink-0 lg:overflow-y-auto">
            <div class="space-y-4">
                {{-- Sidebar Header --}}
                <div class="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-4 shadow-sm">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-lg font-black text-slate-950">Panduan</h2>
                    </div>
                </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
                    {{-- Siswa Group --}}
                    <details class="group/siswa-tree border-b border-slate-100 last:border-b-0" open>
                        <summary
                            class="flex cursor-pointer items-center gap-2 px-4 py-3 text-sm font-black uppercase tracking-wider text-slate-700 transition hover:bg-slate-50">
                            <svg class="size-4 transition-transform group-open/siswa-tree:rotate-90" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7" />
                            </svg>
                            <span class="text-base">🎓</span>
                            <span class="flex-1">Siswa</span>
                            <span
                                class="rounded-full bg-teal-100 px-2 py-0.5 text-xs font-bold text-teal-700">{{ $studentGuides->count() }}</span>
                        </summary>
                        <div class="bg-slate-50/50 py-1">
                            @if ($studentGuides->isNotEmpty())
                                @foreach ($studentGuides as $guide)
                                    <a href="#" data-guide-id="{{ $guide->id }}" data-group="siswa"
                                        @class([
                                            'guide-link-sidebar group/item relative flex items-start gap-2 px-4 py-2 text-sm transition',
                                            'bg-teal-50 font-bold text-teal-950' => $selectedGuideId === $guide->id,
                                        ...
                                        <svg @class([
                                            ...
                                            'text-slate-400' => $selectedGuideId !== $guide->id,
                                ...
                                @endforeach
                            @else
                                <div class="px-4 py-3 text-xs text-slate-500">Belum ada panduan</div>
                            @endif
                        </div>
                    </details>

                    {{-- Panitia Group --}}
                    <details class="group/panitia-tree border-b border-slate-100 last:border-b-0" open>
                        <summary
                            class="flex cursor-pointer items-center gap-2 px-4 py-3 text-sm font-black uppercase tracking-wider text-slate-700 transition hover:bg-slate-50">
                            <svg class="size-4 transition-transform group-open/panitia-tree:rotate-90" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7" />
                            </svg>
                            <span class="text-base">👥</span>
                            <span class="flex-1">Panitia</span>
                            <span
                            ...
                            @if ($committeeGuides->isNotEmpty())
                                @foreach ($committeeGuides as $guide)
                                    <a href="#" data-guide-id="{{ $guide->id }}" data-group="panitia"
                                        @class([
                                            'guide-link-sidebar group/item relative flex items-start gap-2 px-4 py-2 text-sm transition',
                                            'bg-slate-100 font-bold text-slate-950' => $selectedGuideId === $guide->id,
                                            'text-slate-700 hover:bg-white hover:text-slate-700' =>
                                                $selectedGuideId !== $guide->id,
                                        ])>
                                        <svg @class([
                                            'size-4 shrink-0',
                ...
                @if ($selectedGuide && $selectedGuide->toc && count($selectedGuide->toc) > 0)
                    <div id="sidebar-toc" class="rounded-2xl border border-slate-200 bg-white shadow-sm">
                        <div class="border-b border-slate-100 px-4 py-3">
                            <div class="flex items-center gap-2">
                                <div
                                    class="flex size-6 shrink-0 items-center justify-center rounded-lg bg-teal-100 text-sm">
                                    📋
                                </div>
                                <div class="flex-1">
                                    <div class="text-sm font-bold text-slate-950">Daftar Isi</div>
                                    <div class="text-xs text-slate-500">{{ count($selectedGuide->toc) }} bagian</div>
                                </div>
                            </div>
                        </div>
                        <nav id="sidebar-toc-list" class="py-2">
                            @foreach ($selectedGuide->toc as $item)
                                <a href="#{{ $item['id'] }}" @class([
                                    'block px-4 py-2 text-sm font-bold text-slate-700 transition hover:bg-teal-50 hover:text-teal-700',
                                    'pl-6' => $item['level'] === 2,
                                    'pl-8 text-xs font-semibold' => $item['level'] === 3,
                                ])>{{ $item['title'] }}</a>
                            @endforeach
                        </nav>
                    </div>
        ...
        <div class="flex-1 lg:min-w-0">
            ...
            <div class="sticky top-16 z-10 mb-5 space-y-3 lg:hidden">
                    ...
                    @if ($selectedGuide && $selectedGuide->toc && count($selectedGuide->toc) > 0)
                        <details class="group" open>
                            <summary
                                ...
                                <svg class="size-4 text-slate-400 transition-transform group-open:rotate-90"
                ...
                @if ($selectedGuide)
                    <article id="guide-article" class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
                        ...
                        <div id="guide-attachments"
                            class="{{ $selectedGuide->attachments->isNotEmpty() ? '' : 'hidden' }}">
            ...
            <script>
                (() => {
                    const guideLinks = document.querySelectorAll('.guide-link, .guide-link-sidebar');
                    ...
                    guideLinks.forEach(link => {
                        link.addEventListener('click', async (e) => {
                            ...
                            document.querySelectorAll('.guide-link-sidebar').forEach(l => {
                                l.classList.remove('bg-teal-50', 'bg-slate-100', 'font-bold',
                                    'text-teal-950', 'text-slate-950');
                                if (l.getAttribute('data-group') === 'siswa') {
                                    l.classList.add('text-slate-700', 'hover:bg-white',
                                        'hover:text-teal-700');
                                } else {
                                    l.classList.add('text-slate-700', 'hover:bg-white',
                            ...
                            // Apply active styles to clicked link
                            ...
                            } else if (link.classList.contains('guide-link-sidebar')) {
                                // Sidebar link
                                if (isSiswaGuide) {
                                    link.classList.add('bg-teal-50', 'font-bold', 'text-teal-950');
                                    link.classList.remove('text-slate-700', 'hover:bg-white',
                                        'hover:text-teal-700');
                                } else {
                                    link.classList.add('bg-slate-100', 'font-bold', 'text-slate-950');
                                    link.classList.remove('text-slate-700', 'hover:bg-white',
                                        'hover:text-slate-700');
                                }
                            }

                            // Also activate corresponding link in the other navigation
                            const correspondingLinks = document.querySelectorAll(
                                `[data-guide-id="${guideId}"]`);
                            correspondingLinks.forEach(correspondingLink => {
                                if (correspondingLink !== link) {
                                    if (correspondingLink.classList.contains('guide-link')) {
                                        if (isSiswaGuide) {
                                            correspondingLink.classList.add('bg-teal-50',
                                                'font-black', 'text-teal-950', 'guide-active');
                                        } else {
                                            correspondingLink.classList.add('bg-slate-100',
                                                'font-black', 'text-slate-950', 'guide-active');
                                        }
                                        correspondingLink.classList.remove('hover:bg-slate-50');
                                    } else if (correspondingLink.classList.contains(
                                            'guide-link-sidebar')) {
                                        if (isSiswaGuide) {
                                            correspondingLink.classList.add('bg-teal-50',
                            ...
                            try {
                                ...
                                // Update TOC (both mobile and sidebar)
                                if (data.toc && data.toc.length > 0) {
                                    ...
                                    if (sidebarToc) {
                                        ...
                                        if (sidebarTocList) {
                                            sidebarTocList.innerHTML = data.toc.map(item => `
                                                <a href="#${item.id}" class="block px-4 py-2 text-sm font-bold text-slate-700 transition hover:bg-teal-50 hover:text-teal-700 ${item.level === 2 ? 'pl-6' : ''} ${item.level === 3 ? 'pl-8 text-xs font-semibold' : ''}">
                                                    ${item.title}
                                                </a>
                                            `).join('');
                                        }
                                    ...
                                    if (sidebarToc) sidebarToc.classList.add('hidden');
                                }
